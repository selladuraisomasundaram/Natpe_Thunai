...
interface PostFoodListingFormProps {
  onSubmit: (data: any) => void;
  onCancel: () => void;
  isCustomRequest?: boolean; // Toggle between "Selling" and "Requesting"
}

const PostFoodListingForm: React.FC<PostFoodListingFormProps> = ({ 
  onSubmit, 
  onCancel, 
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (!title || !description || !price || !timeEstimate) {
      toast.error("Please fill in all the details to help others.");
      return;
    }
    ...
    const data = {
      title,
      description,
      price: `₹${price}`, // Format as currency string for consistency
      category,
    ...
    <form onSubmit={handleSubmit} className="space-y-4 py-2">
      ...
      <div className="space-y-2">
        ...
        <RadioGroup defaultValue="veg" value={dietaryType} onValueChange={setDietaryType} className="flex gap-4">
            <div className="flex items-center space-x-2 border border-border p-2 rounded-md cursor-pointer hover:bg-green-500/10 transition-colors">
            ...
            <div className="flex items-center space-x-2 border border-border p-2 rounded-md cursor-pointer hover:bg-red-500/10 transition-colors">
...
export default PostFoodListingForm;