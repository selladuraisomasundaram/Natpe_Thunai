...
interface PostLostItemFormProps {
  onItemPosted: () => void;
  onCancel: () => void;
}

const PostLostItemForm: React.FC<PostLostItemFormProps> = ({ onItemPosted, onCancel }) => {
    ...
    <form onSubmit={handleSubmit} className="grid gap-4 py-4">
      ...
      <div className="space-y-2">
        ...
        <Input
          ...
          placeholder="e.g., https://example.com/lost-bottle.jpg"
          className="bg-input text-foreground border-border focus:ring-ring focus:border-ring"
          disabled={isSubmitting}
        />
        <p className="text-xs text-muted-foreground mt-1">A clear image can help others identify your item.</p>
      </div>
      <div className="space-y-2">
        <Label htmlFor="location" className="text-foreground">Last Seen Location</Label>
        <Input
          id="location"
          type="text"
          value={location}
      ...
      <div className="space-y-2">
        <Label htmlFor="date" className="text-foreground">Date Lost</Label>
        <Input
          id="date"
          type="date"
          value={date}
      ...
      <DialogFooter className="pt-4 flex flex-col sm:flex-row gap-2">
        ...
        <Button type="submit" disabled={isSubmitting} className="w-full sm:w-auto bg-secondary-neon text-primary-foreground hover:bg-secondary-neon/90">
          {isSubmitting ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : "Post Lost Item"}
        </Button>
      </DialogFooter>
    </form>
  );
...
export default PostLostItemForm;